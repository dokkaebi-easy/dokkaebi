...
public class UserService {
    private final FileMannagerUtil fileMannagerUtil;
    private final UserRepository userRepository;
    String userFileName = "userinfo";

    public void signUp(UserDto userDto) throws IOException, AroundHubException {
        if (userRepository.findByPrincipal(userDto.getPrincipal()).isPresent()){
            log.error("중복된 아이디");
            throw new AroundHubException(ExceptionClass.USER, HttpStatus.BAD_REQUEST,"이미 가입된 아이디입니다");
        }
    ...
    public UserDto logIn(UserDto userDto) throws IOException, AroundHubException {
        ...
        if (user.get().equals(userDto)){
            ...
            throw new AroundHubException(ExceptionClass.USER, HttpStatus.BAD_REQUEST,"비밀번호 혹은 아이디가 일치 하지 않습니다");
        }
    }
